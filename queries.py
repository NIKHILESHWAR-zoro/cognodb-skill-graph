...
def skill_gap_jobs(person_id: str, limit: int = 10):
    ...
    return run_query(
        ...
        WHERE NOT (p)-[:HAS_SKILL]->(bridge)
        WITH j, c, collect(DISTINCT bridge.name) AS bridgeSkills, count(DISTINCT bridge) AS gapCount
        RETURN j.id AS job_id, j.title AS title, c.name AS company, bridgeSkills, gapCount
        ORDER BY gapCount ASC
        LIMIT $limit
        """,
        {"pid": person_id, "limit": limit},
    )
